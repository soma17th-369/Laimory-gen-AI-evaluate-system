...
### M3 — 리포트 ✅ (코드; 세션 채점 결과 조립·내보내기)
- [x] 채점 결과 → 트레이스별 리포트 조립 + 세션 집계(기준별 평균·심각도별 건수)
- [x] 화면 요약 표시 + 내보내기(md/json 다운로드)

### M4 — 활용 ✅ (코드; a 프롬프트 개선 / b 테스트 케이스)
- [x] 리포트 기반 프롬프트 개선 제안 — generation 실제 프롬프트(messages) 대조 → 단계별 수정안
- [x] 리포트 기반 테스트 케이스 생성 — {입력 스냅샷(단순화) + 기대 조건} + json 내보내기
